  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateInfo({
        accessToken,
        id: data?.data[0].id,
        data: formDataToSend , // Sao chép dữ liệu để đảm bảo không bị thay đổi ngoài ý muốn
      }).unwrap();
        ...
        <div className="w-full max-w-lg">
          <form onSubmit={handleSubmit} className="flex flex-col items-center">
            ...
            <div className="w-48 h-48 mt-4">
              <img
                src={"https://res.cloudinary.com/dd8yzbulm/" + data?.data[0].avatar} // Dùng giá trị từ state
                alt="Avatar"
                className="w-48 h-48 object-cover rounded-full border border-black"
              />
            </div>
